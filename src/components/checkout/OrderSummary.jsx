  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-xl h-fit border border-gray-100 sticky top-4">
      ...
      <div className="space-y-3 pb-4 border-b mb-4">
        {cart.items.map((item) => (
          ...
          >
            <img
              src={
                productImageMap[item.hinhAnh] ||
                "https://placehold.co/40x40?text=SP"
              }
              alt={item.tenSanPham}
      ...
      <div className="space-y-2 text-gray-700 text-sm">
        <div className="flex justify-between">
          ...
          <span className="font-medium">
            {(total - phiVanChuyen + giamGiaKM + diemTienMat).toLocaleString()}đ
          </span>
        </div>
        <div className="flex justify-between">
      ...
      <button
        type="button"
        onClick={onConfirm}
        className={`mt-6 w-full bg-orange-500 text-white px-4 py-3 rounded-lg font-bold transition-colors ${
