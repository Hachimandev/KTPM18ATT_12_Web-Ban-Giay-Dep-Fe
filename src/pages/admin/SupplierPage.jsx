...
const SupplierPage = () => {
  ...
  const fetchSuppliers = async () => {
    try {
      const res = await fetch("http://localhost:8085/api/suppliers");
...
//       if (!res.ok) throw new Error("Xóa nhà cung cấp thất bại");

//       setSuppliers(suppliers.filter((s) => s.maNhaCungCap !== id));
        ...
        <table className="min-w-full border-collapse">
          ...
          <tbody>
            ...
            paginatedSuppliers.map((s) => (
              <tr key={s.maNhaCungCap} className="border-b hover:bg-gray-50 text-sm">
                <td className="p-3">
                  <input type="checkbox" />
                </td>
                <td className="p-3">
                  <div className="flex flex-col">
                    <span className="font-medium text-gray-800">{s.tenNhaCungCap}</span>
                    <span className="text-xs text-gray-500">Mã: {s.maNhaCungCap}</span>
                  </div>
                </td>
                <td className="p-3">
                  <div className="flex flex-col">
                ...
                <td className="p-3 flex justify-center gap-2">
                  ...
                  onClick={() =>
                              navigate(
                                `/admin/suppliers/edit/${s.maNhaCungCap}`
                              )
                            }
