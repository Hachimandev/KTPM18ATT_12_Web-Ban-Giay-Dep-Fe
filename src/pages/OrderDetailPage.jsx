...
import { FiArrowLeft, FiClock, FiCheckCircle, FiTruck, FiXCircle } from 'react-icons/fi';
...
const OrderDetailPage = () => {
    ...
    const totalItems = order.chiTietHoaDons?.reduce((sum, item) => sum + item.soLuong, 0) || 0;
    const statusDetails = statusMap[order.trangThaiHoaDon] || { label: order.trangThaiHoaDon, color: 'bg-gray-100 text-gray-700', icon: null };
        ...
        <div className="min-h-screen bg-gray-50 p-6 md:p-10">
            <div className="max-w-4xl mx-auto bg-white shadow-xl rounded-xl p-6 md:p-10">
                ...
                <div className="flex justify-between items-start border-b pb-4 mb-6">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-800">Chi tiết đơn hàng: {order.maHoaDon}</h2>
                        <p className="text-gray-500 mt-1">Ngày đặt: {new Date(order.ngayDat).toLocaleDateString('vi-VN')}</p>
                    ...
                    <div className={`flex items-center px-3 py-1 rounded-full text-xs font-semibold ${statusDetails.color}`}>
                        {statusDetails.icon} <span className="ml-1">{statusDetails.label}</span>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    {order.chiTietHoaDons.map((item) => (
                        <div key={item.maChiTietHoaDon} className="flex items-center p-4 border border-gray-200 rounded-lg hover:shadow-sm transition">
                            {/* Hình SP (placeholder) */}
                            <img
                                src={productImageMap[item.sanPham.hinhAnh] || `https://placehold.co/60x60?text=${item.sanPham?.tenSanPham.substring(0, 3) || 'SP'}`}
                                alt={item.sanPham?.tenSanPham}
                                className="w-16 h-16 rounded-md object-cover"
                            />

                            <div className="ml-4 flex-1">
                                <p className="font-medium text-gray-800">{item.sanPham?.tenSanPham}</p>
                            ...
                            <div className="text-right">
                                <p className="text-orange-600 font-bold">{item.tongTien.toLocaleString()} VNĐ</p>
                ...
                <div className="mt-6 flex justify-end">
                    <p className="text-xl font-bold text-gray-800">
                        Tổng cộng: <span className="text-orange-600">{order.thanhTien.toLocaleString()} VNĐ</span>
                    </p>
                </div>
            </div>
        </div>
